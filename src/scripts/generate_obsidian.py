"""Obsidian theme builder. Generated from glauca.json.
...
def _named_hues(pal):
    """8 working hues + the one identity mark, mode-independent (like the
...
def _ramp(m):
    bg, surf, raised, bord, txt, muted, seab = (m["bg"], m["surface"], m["surface-raised"],
        m["border"], m["text"], m["text-muted"], m["tint-bright"])
...
def _task_rules():
    ...
    per-mode --gl-task-* vars emitted in blk(); done [x] and the untouched [ ] keep core styling."""
    ...
    glyph_css = ('  content: "%s"; position: static; display: flex; align-items: center; justify-content: center;\n'
                 ...
                 "  color: var(--gl-task-%s); font-family: var(--gl-font-sans);\n"
                 "  font-size: calc(var(--checkbox-size) * 0.72); font-weight: 700; line-height: 1;")
    out = []
    for ch, key, glyph in _TASK_GLYPHS:
        out.append("%s {\n  background-color: transparent; border-color: var(--gl-task-%s);\n}" % (sels(ch), key))
        out.append("%s {\n%s\n}" % (sels(ch, True), glyph_css % (glyph, key)))
    out.append("%s {\n  background-color: transparent; border-color: var(--gl-task-progress);\n}" % sels("/"))
    out.append('%s {\n  content: ""; position: static; display: block; width: 100%%; height: 100%%;\n'
               "  -webkit-mask-image: none; border-radius: inherit;\n"
               "  background: linear-gradient(90deg, var(--gl-task-progress) 50%%, transparent 50%%);\n}" % sels("/", True))
...
_FEATURES_CSS = """\
/* Focus mode (Style Settings: gl-focus). Chrome recedes until pointed at; the note stands alone.
   Restores on hover or keyboard focus within. */
body.gl-focus :is(.workspace-tab-header-container, .view-header, .workspace-ribbon, .status-bar) {
  opacity: 0.25; transition: opacity 0.2s ease;
}
body.gl-focus :is(.workspace-tab-header-container, .view-header, .workspace-ribbon, .status-bar):is(:hover, :focus-within) {
  opacity: 1;
}
/* Seamless embeds (default; Style Settings gl-embed-frames restores the framed look). Transclusions
   read as part of the note: no tint, border or title; the open-link affordance appears on hover. */
body:not(.gl-embed-frames) .markdown-rendered .markdown-embed,
body:not(.gl-embed-frames) .markdown-source-view .markdown-embed {
  background-color: transparent; border: none; padding: 0;
}
body:not(.gl-embed-frames) .markdown-embed-title { display: none; }
body:not(.gl-embed-frames) .markdown-embed-link { opacity: 0; transition: opacity 0.15s ease; }
body:not(.gl-embed-frames) .markdown-embed:hover .markdown-embed-link { opacity: 1; }
...
.markdown-rendered .image-embed[alt]:not([alt=""]):not([alt$=".png" i]):not([alt$=".jpg" i]):not([alt$=".jpeg" i]):not([alt$=".webp" i]):not([alt$=".gif" i]):not([alt$=".svg" i]):not([alt$=".bmp" i])::after {
  content: attr(alt); display: block; font-family: var(--gl-font-sans); font-size: 0.82em;
...
def _properties_map(m):
    ...
    below body text: IBM Plex Mono at the same nominal size reads larger than
...
def _settings_block(lit, cold):
    ...
    Glauca voice, so a fresh install (plugin absent, or present but
...
def build_obsidian(D):
    ...
    def blk(m, sel):
        ...
        L += [
         ...
         # set explicitly below -- so this var is free to do its real job again. Same tint-bright family as
         # --nav-item-background-hover (0x1f), one step under --background-modifier-active-hover's 0x33.
         "  --background-modifier-hover: %s1f;" % m["tint-bright"],
         ...
         # private --gl-blockquote-bg plus a hand-rolled background on those same selectors.
         "  --blockquote-background-color: %s1f;" % m["tint-bright"],
         "  --background-modifier-active-hover: %s33;" % m["tint-bright"],
        ...
        L += [
         ...
         "  --gl-task-sched: %s;" % _light_safe_hue(named["cyan"], m),
         "  --gl-task-question: %s;" % _light_safe_hue(named["purple"], m),
         ...
         "  --pill-background: transparent;", "  --pill-background-hover: %s33;" % m["tint-bright"],
         ...
         "  --nav-item-color-active: %s;" % m["text"], "  --nav-item-background-hover: %s1f;" % m["tint-bright"],
         ...
         # a tint-bright tint is visible in both, and stronger than --nav-item-background-hover's tint
         # so the persistently-selected note reads as more prominent than a transient hover.
         "  --nav-item-background-active: %s33;" % m["tint-bright"],
         ...
         "  --scrollbar-bg: transparent;", "  --scrollbar-thumb-bg: %s40;" % m["tint-bright"],
         "  --scrollbar-active-thumb-bg: %s66;" % m["tint-bright"],
         ...
         "  --table-row-background-hover: %s14;" % m["tint-bright"], "  --table-selection: %s22;" % m["accent"],
         "  --table-selection-border-color: var(--text-accent);",
         "  --table-header-background: %s;" % m["surface-raised"], "  --table-header-background-hover: %s33;" % m["tint-bright"],
    ...
    fx = 'body:not(.gl-nav-plain) .workspace-leaf-content[data-type="file-explorer"]'
    ...
    nav_icons = (
        "/* File-explorer icons (Style Settings: gl-nav-plain removes them). Folders swap closed/open\n"
    ...
    return (_settings_block(lit, cold) +
            ...
            "body {\n"
            '  --gl-font-serif: "IBM Plex Serif", Georgia, serif;\n'
            '  --gl-font-sans: "IBM Plex Sans", system-ui, sans-serif;\n'
            '  --gl-font-mono: "IBM Plex Mono", ui-monospace, monospace;\n'
            "  /* Literal stacks here, not var(--gl-font-*): these three are Obsidian's own theme-level\n"
            ...
            '  --font-text-theme: "IBM Plex Sans", system-ui, sans-serif;\n'
            '  --font-interface-theme: "IBM Plex Sans", system-ui, sans-serif;\n'
            ...
            "  font-size: calc(1em * var(--gl-body-scale, 1.2));\n"
            ...
            ".metadata-property-key, .metadata-property-value { font-family: var(--gl-font-mono); }\n"
            ...
            'body.gl-explorer-truncate .workspace-leaf-content[data-type="file-explorer"] .tree-item-inner {\n'
