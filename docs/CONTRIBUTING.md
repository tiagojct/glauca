...
from glauca.json. Do not edit generated files by hand; your change would be
overwritten on the next `make generate` and the CI drift check would fail.

To change a token: edit glauca.json, then run `make generate`, then
...
Add the data to src/glauca.json, then teach src/scripts/generate.py to emit it
