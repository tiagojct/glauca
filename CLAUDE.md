...
- **Two modes, light-first**: `lit` (Profundum, dark, `scheme: dark`) and `cold` (Pruina, light, `scheme: light`). They must keep identical token keys — `validate.py` enforces parity. CSS emits `cold` as `:root` / `[data-mode="cold"]`, `lit` as `[data-mode="lit"]`. Plain file names (`Glauca.itermcolors`) are the light variant; dark carries `-Dark`.
- **Tiers**: `core` hues (saxum, glaucum, caelum, pruina) are the identity and appear everywhere; `extended` hues (folium, bacca, viola, lacus, unda) exist **only** for code/terminal (folium also anchors dataviz) and must not appear on posters, slides, or web.
